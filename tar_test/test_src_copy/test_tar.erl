...
dir(DirName,[{_,TarDir}])->
    io:format(" ~p~n",[{start, ?MODULE,?LINE}]), 
    io:format(" ~p~n",[{"DirName",":=> ",DirName,?MODULE,?LINE}]), 
%    io:format(" ~p~n",[{"TarDir",":=> ",TarDir,?MODULE,?LINE}]),  
   
    % Create a tar file
%    {ok,Files}=file:list_dir(DirName),
%    TarFileName=filename:basename(DirName)++".tar",
%    ok=erl_tar:create(TarFileName,Files),
    
    % Creat if needed dir to  put the tar file obs basename
    NewTarDir=filename:join([TarDir,filename:basename(DirName)]),
  %  io:format(" ~p~n",[{"NewTarDir",":=> ",NewTarDir,?MODULE,?LINE}]),  
    case filelib:is_dir(NewTarDir) of
	true-> % already exists
	    ok;
	false->
 ...
 %   {ok,_}=file:copy(filename:join([DirName,TarFileName]),NewTarDir),
  %  io:format("dir = ~p~n",[{DirName,Files,?MODULE,?LINE}]),
  %  io:format(" ~p~n",[{"[{DirName,NewTarDir}|Acc]",":=> ",[{DirName,NewTarDir}|Acc],?MODULE,?LINE}]), 
    io:format(" ~p~n",[{stopp, ?MODULE,?LINE}]), 
    [{DirName,NewTarDir}].

regular(FileName,Acc)->
  %  io:format("regular = ~p~n",[{FileName,?MODULE,?LINE}]),
    Acc.
...
%    SourceDir="/home/pi/erlang/tar_test/c/source/adder_service",
    ...
    TarDir="/home/pi/erlang/tar_test/tar_dir",

    {RegularResult,DirResult}=dfs:search(SourceDir,?MODULE,[],[{SourceDir,TarDir}]),
    io:format(" ~p~n",[{"RegularResult",":=> ",RegularResult,?MODULE,?LINE}]), 
    io:format(" ~p~n",[{"DirResult",":=> ",DirResult,?MODULE,?LINE}]), 
    VisitedDirs=[DirName||{DirName,_}<-DirResult],
  %  io:format(" ~p~n",[{"VisitedDirs",":=> ",VisitedDirs,?MODULE,?LINE}]), 
   %  io:format(" ~p~n",[{"DfsResult",":=> ",DfsResult,?MODULE,?LINE}]), 
    init:stop().    
    % clean up
